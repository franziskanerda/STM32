...
// mit 10K. Die Berechnung der Zwischenwerte erfolgt �ber einfache lineare Interpolation
...
// AD-Wandler-Schwellwerte fuer die K�hlk�rpertemperatur. Berechnung: (Wert aus Datenblatt
// NCP18XH103F03RB) / (Wert + 10) * 4096. Datenblatttabelle auf 10K bezogen, Spannungsteiler
// mit 10K. Die Berechnung der Zwischenwerte erfolgt �ber lineare Interpolation
...
void SageTsk_CheckForAction( void ) 
{
	...
	if ( ( (sys_tic % 400) == 0 ) || (PowerState == POWEROFF_NOTCHARGING_LOOP) )
	{
		...
		switch (PowerState)
		{
			...
			{
				// Hardware f�r den Ladezustand richtig initialisieren
				PowerOffCharging ();
				I2C_Configuration ();
				DbgPulse (2);
				DelayUsRaw (20000);
			...
			case POWERON_NOTCHARGING_INIT:
				// Hardware f�r den eingeschalteten Zustand initialisieren
...
void DoSysAdMeasurements( void )
{
	...
	// Umrechnung von A/D-Stufen (16 Werte) auf Temperatur (K�hlk�rpertemperatur)
	...
	// Helligkeitswert direkt �bernehmen (0...65536)
