...
/** Description        *******************************************************************************/
// USART1 wird f�r RS232 verwendet.
...
die zu �bertragende L�nge in der DMA-Engine auf die Gr�sse de Empfangspuffers gesetzt. Eine Ende-
Erkennung erfolgt durch DMA nicht weil im vorraus ja unbekannt ist wie lange die zu empfangenden
Daten sind. F�r die Ende-Erkennung werden die UARTs so konfiguriert das bei einer Idle-Bedingung
(=RX-Leitung ist nach einem empfangegem Zeichen 1.5 x Zeit f�r ein Byte ruhig) oder bei einem Fehler im
empfangenem Zeichen ein IRQ generiert wird (USART1_IRQHandlerUSART4_IRQHandler).
Der Irq stellt den Empfangstask dann auf "RxTsk_CheckForTele". Der Empfangstask pr�ft dann den
Empfangsbuffer auf ein vollst�ndig angekommenes Telegramm (CheckForTele) und veranlasst
die Aktion (DoTele).
*/
...
typedef struct TxBufStrct_s
{
	...
	u32 datapos;               	// Positionsspeicher. Anzahl der bereits �bertragenen Daten von buf
...
/******************************************************************************************************
                                             SendTxBuffer
-------------------------------------------------------------------------------------------------------
Schickt den tx_buf �ber den angegebenen Kommunikationskanal
...
s16 SendTxBuffer( u8 Channel )
{
   ...
   // DMA und passenden IRQ f�r den DMA-Abschluss konfigurieren
...
/******************************************************************************************************
                                               SendData
-------------------------------------------------------------------------------------------------------
Schickt Daten raw �ber den Kommunikationskanal
...
/******************************************************************************************************
                                               SendTeleCh
-------------------------------------------------------------------------------------------------------
Sendet ein Telegramm �ber den angegebenen Kommunikationskanal
...
Parameter:
   ...
   DataLen:  L�nge der zu kodierenden Daten in byte
   DestPtr:  Zeiger auf Zeiger wohin die kodierten Daten geschrieben werden sollen. Der Zeiger wird
             entsprechend der kodierten Datenmenge weitergef�hrt.
...
/******************************************************************************************************
                                             SendTeleFirst
-------------------------------------------------------------------------------------------------------
Erster Teil einer Telegramm�bertragung. Schreibt das Startzeichen und den Header in tx_buf.
...
s16 SendTeleFirst( u16 Tid, RxBufStrct *rxb )
{
   ...
   *dp++ = RS_START_CHAR;
   ...
   tx_buf.datalen = (u32)(dp - tx_buf.buf); // Telegramml�nge ausrechnen
...
/******************************************************************************************************
                                               SendTeleNext
-------------------------------------------------------------------------------------------------------
N�chster Teil einer Telegramm�bertragung. Schreibt einen weiteren Datenblock nach tx_buf.
...
s16 SendTeleNext( const void *Data, u32 DataLen )
{
   ...
   // CRC �ber die Daten weiterrechnen
...
/******************************************************************************************************
                                               SendTeleLast
-------------------------------------------------------------------------------------------------------
Letzter Teil einer Telegramm�bertragung. Schreibt die CRC und das Stopzeichen und sendet ab.
...
void DMA1_Channel4_IRQHandler( void )
{
   ...
   {
      USART_ClearFlag( USART1, USART_FLAG_TC ); // Transmission completed Flag l�schen
      tx_buf.status = TXBUF_IDLE;
...
/******************************************************************************************************
                                             ResetRxBuffer
-------------------------------------------------------------------------------------------------------
Setzt die rx_buf Struktur zur�ck
...
/******************************************************************************************************
                                          RxTsk_CheckForTele
-------------------------------------------------------------------------------------------------------
Wird von den Empfangs-IRQs aktiviert wenn die RX-Leitung einen Idle-Zustand erkennt. Dann wird gepr�ft
ob ein Telegramm vollst�ndig angekommen ist und wenn ja darauf geantwortet
******************************************************************************************************/
...
/******************************************************************************************************
                                             CheckForTele
-------------------------------------------------------------------------------------------------------
Pr�ft auf dem angegebenen Kanal ob ein Telegramm vollst�ndig angekommen ist.
Wenn eine Telegramm angekommen ist stehen in rxb->rdptr die empfangenen Daten, rxb->cnt enth�lt die
Anzahl der empfangenen Daten in Byte. In rxb->tid steht die empfangene Telegrammnummer, rxb->channel
enth�lt den Kanal auf dem empfangen wurde.

Parameter:
   Channel:  siehe #defines COM_CHANNEL_x

Return: 0 - vollst�ndiges Telegramm angekommen.
...
s16 CheckForTele( u8 Channel )
{
   ...
   if ( cc->dma_rx != NULL )
   {
      // Ausrechnen wieviele neue Bytes emfangen wurden. dma_rx->CNDTR ist der r�ckw�rtsz�hlende Counter
      // der DMA-Hardware (noch zu �bertragende Bytes, wird in ResetRxBuffer auf sizeof(rxb->buf)
      // initialisiert)
      rx_len = rxb->dma_cntr - cc->dma_rx->CNDTR;
      rxb->dma_cntr = cc->dma_rx->CNDTR;                 // Z�hlerstand der Hardware merken f�rs n�chste mal
   }
   else
   {
      ...
      case COM_MODE_TELE:
         while ( rx_len-- > 0 )
         {
            ...
            if( data == RS_START_CHAR )
            {
               ...
               rxb->cnt = 0;
               rxb->ESC_received = 0;
               TimeoutStart( rxb->timeout_time );     // Start Timeout f�r den vollst�ndigen Empfang
            }
            ...
            else if( data == RS_STOP_CHAR )
            {
               ...
               // Es m�ssen wenigstens ein Header und eine CRC empfangen worden sein
               ...
               // Seriennummer und DeviceClass pr�fen. Wert 0 oder richtiger Wert wird jeweils akzeptiert.
               ...
               // Channel und tcn werden im Fehlerfall f�rs Nack gebraucht deshalb schon hier speichern
               rxb->channel = Channel;
               rxb->tcn = head->tcn;
               ...
               // im Datenbereich arbeitet sind die dann zuverl�ssig abgeschlossen.
               *rxb->rdptr = 0;
               ...
               // Pr�fen ob das angekommene Telegram eine Aufforderung zum Resend des letzten
...
/******************************************************************************************************
                                            CheckForCommTimeout
-------------------------------------------------------------------------------------------------------
Wird vom Sage-Task alle 10ms aufgerufen. Pr�ft ob einer der Kan�le mit einem unfertigen Telegramm
ins Timeout l�uft und resettet den Kanal wenn n�tig.
******************************************************************************************************/
void CheckForCommTimeout( void ) {
   ...
   for ( n = 0 ; n < NUMBER_OF_COM_CHANNELS ; n++ )
   {
      ...
      // Pr�fen ob ein Timeout w�hrend des Empfangs aufgetreten ist
